...
error() {
    ...
    echo "Usage: $0 [--port <port>] [--max-model-len <length>] [--enable-lora] [--prefix-caching] [--kv-fp8] [--enforce-eager]"
    exit 1
}
...
--max-num-seqs $max_num_seqs --disable-log-requests --max-num-batched-tokens $max_num_batched_tokens \
