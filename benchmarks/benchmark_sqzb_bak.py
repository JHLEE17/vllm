...
def main(args: argparse.Namespace):
    ...
    decode_batch_sizes = [n for label, n in iteration_data['batch_sizes'] if label == 'd']
    mean_batch_size = sum(decode_batch_sizes) / len(decode_batch_sizes)
    reset_running_bs(url + "/clear_iteration_data")
    ...
    print(f"\tMean running batchsize: {mean_batch_size}")
    ...
    df['ttft'] = (df['first_token_time'] - df['first_scheduled_time']) * sec_to_msec

    # TPOT(Time Per Output Token) 계산 및 컬럼 추가
    df['tpot'] = (df['finished_time'] - df['first_token_time']) * sec_to_msec / df['num_generated_tokens']
    ...
    out_path += f"_max_max_num_batched_tokens{args.max_num_batched_tokens}"
    out_path += ".pkl"
    ...
    if args.csv_path:
        ...
        # csv_path += f"_out{total_generated_tokens}"
        csv_path += f"_guided" if args.json_template else ""
        csv_path += f"_LoRA" if args.lora_pattern else ""
        csv_path += f"_prefix_caching" if args.prefix_caching else ""
        csv_path += "_Pr" if args.max_output_len == 1 else "_De"
        csv_path += f"_max_max_num_batched_tokens{args.max_num_batched_tokens}"
        csv_path += ".csv"
        ...
        summary_data = {
            ...
            "Value": [args.num_requests, total_input_tokens, total_generated_tokens, f"{benchmark_time*1000}",# mean_batch_size,
